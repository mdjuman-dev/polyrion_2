$(function () {
  ...
  $(document).ready(function () {
      ...
      $scrollLeft.on('click', function () {
        if ($filtersSection.length > 0) {
          $filtersSection.animate(
            { scrollLeft: $filtersSection.scrollLeft() - scrollAmount },
            {
              duration: 300,
              complete: function () {
                updateScrollButtons();
    ...
    $('.frequency-btn').on('click', function (e) {
      ...
      // Adjust dropdown position to prevent overflow
      if ($wrapper.hasClass('active')) {
        adjustDropdownPosition($wrapper.find('.frequency-dropdown-menu'));
      }
    });
  ...
  $('.nav-item').on('click', function (e) {
    ...
    if (
      !$(this).closest('.more-dropdown-menu, .profile-nav-dropdown-menu').length
    ) {
  ...
  $('.profile-nav-dropdown-menu .dropdown-item').on('click', function (e) {
    ...
    if (href && href.includes('user dashboard.html')) {
      // Update active state
      $('.profile-nav-dropdown-menu .dropdown-item').removeClass('active');
      $(this).addClass('active');
  ...
  // More Menu Toggle
  $('#moreMenuBtn, #closeMenuBtn, #moreMenuOverlay').on('click', function (e) {
  ...
  function showMarketResults(searchTerm, $container) {
    ...
    marketResults.forEach(function (result) {
      ...
      resultHtml +=
        '<div class="mobile-search-result-icon">' + result.icon + '</div>';
...
$(document).ready(function () {
  ...
  function handleHashNavigation() {
    ...
    if (
      ...
      $(`.mobile-profile-dropdown-item[data-tab="${hash}"]`).addClass('active');
      $(`.profile-nav-dropdown-menu .dropdown-item[href*="${hash}"]`).addClass(
        'active'
      );
...
$(document).ready(function () {
  ...
  function generateChartData(target, length) {
    ...
    for (let i = 0; i < length; i++) {
      current =
        current + (Math.random() - 0.5) * 10 + (target - current) * 0.02;
  ...
  function updateOutcomePrice() {
    ...
    if ($outcomePrice.length) {
      if (
        ...
        $outcomePrice.text(
          isBuy ? (isYes ? '0.1¢' : '99.9¢') : isYes ? '99.9¢' : '0.1¢'
        );
      }
    }
  }
  ...
  $('#confirmTrade').click(function () {
    ...
    setTimeout(() => {
      $('#executeTrade').text(original).css('background', 'var(--accent)');
    }, 2000);
    currentShares = 0;
    $('#sharesInput').val('0');
    $('#limitPrice').val('');
...
$(function () {
  ...
  $avatarWrapper.on('click', function () {
    ...
    changeAvatarImage(imageSources[currentIndex], './assets/images/user.webp');
  });
