...
@section('meta_derails')
   <title>{{ $market->question }} - {{ $appName }}</title>
   <meta name="description" content="{{ Str::limit($market->description ?? $market->question, 160) }}">
   <meta property="og:title" content="{{ $market->question }} - {{ $appName }}">
   <meta property="og:description" content="{{ Str::limit($market->description ?? $market->question, 160) }}">
   <link rel="canonical" href="{{ $appUrl }}/market/{{ $market->slug ?? $market->id }}">
@endsection
@section('content')
   <style>
         ...
         <div class="main-content">
            <div class="market-detail-header">
               <div class="market-header-top">
                  <div class="market-header-left">
                     ...
                     <div class="market-header-info">
                        ...
                        <div class="market-header-meta">
                           <span class="market-volume">${{ number_format($market->volume ?? 0) }} Vol.</span>
                           @if($market->end_date)
                              <span class="market-date">{{ format_date($market->end_date) }}</span>
                  ...
                  @if (($market->end_date ?? $event->end_date) && \Carbon\Carbon::parse($market->end_date ?? $event->end_date)->diffInDays(now()) < 30)
                     <div class="countdown-container"
                        data-end-date="{{ \Carbon\Carbon::parse($market->end_date ?? $event->end_date)->toIso8601String() }}">
            ...
            {{-- Outcomes section hidden for single market --}}
            ...
            ">
               ...
               <div class="rules-content" id="rulesContent" style="
                  ...
                  padding-bottom: 1rem;
               ">
                  <div style="white-space: pre-wrap; word-wrap: break-word;">{{ $market->description ?? $event->description }}</div>
                  
                  @if($market->created_at ?? $event->created_at)
                  <div style="
                     margin-top: 1.5rem;
                     padding-top: 1rem;
                     ...
                     color: rgba(255, 255, 255, 0.6);
                  ">
                     <strong style="color: rgba(255, 255, 255, 0.8);">Created At:</strong> {{ ($market->created_at ?? $event->created_at)->format('M d, Y, g:i A') }} {{ ($market->created_at ?? $event->created_at)->timezoneName }}
                  </div>
                  @endif
               </div>
            </div>
            ...
            let option = {
               ...
               series: seriesData.map((item, index) => {
                  ...
                  return {
                     ...
                     data: item.data,
                     lineStyle: {
                        width: 3, // Thicker line for single market
                        color: validColor
                     },
                     areaStyle: {
   ...
   @push('script')
      <script>
         // Initialize trading panel for single market page
         // This ensures trading calculations work on single market page
         (function() {
            function initializeSingleMarketTrading() {
               // Wait for trading panel to be loaded (Livewire component)
               const checkTradingPanel = setInterval(function() {
                  ...
                  clearInterval(checkTradingPanel);
                  
                  // Extract prices from buttons (prices are in decimal format 0-1)
                  const yesPriceDecimal = parseFloat(yesBtn.getAttribute('data-price')) || 0.5;
                  const noPriceDecimal = parseFloat(noBtn.getAttribute('data-price')) || 0.5;
                  
                  // Convert to cents for display (0.5 = 50¢)
                  // Frontend layout expects prices in cents
                  window.currentYesPrice = yesPriceDecimal * 100;
                  window.currentNoPrice = noPriceDecimal * 100;
                  
                  // Get outcomes
                  window.currentFirstOutcome = yesBtn.getAttribute('data-outcome') || 'Yes';
                  window.currentSecondOutcome = noBtn.getAttribute('data-outcome') || 'No';
                  
                  // Get market ID
                  const tradingPanel = document.getElementById('tradingPanel');
                  if (tradingPanel) {
                     window.currentMarketId = tradingPanel.getAttribute('data-market-id');
                  }

                  console.log('Single market trading initialized', {
                     yesPrice: window.currentYesPrice,
                     noPrice: window.currentNoPrice,
                     firstOutcome: window.currentFirstOutcome,
                     secondOutcome: window.currentSecondOutcome,
                     marketId: window.currentMarketId
                  });

                  // Trigger updateSummary if it exists (from frontend layout)
            ...
            if (document.readyState === 'loading') {
               document.addEventListener('DOMContentLoaded', initializeSingleMarketTrading);
            } else {
               initializeSingleMarketTrading();
            }

            // Also initialize when Livewire loads
            document.addEventListener('livewire:load', initializeSingleMarketTrading);
         })();

         // Make updateShares function globally available for quick buttons
         // This is called by the +$1, +$20, etc. buttons
         window.updateShares = function(amount) {
            ...
            sharesInput.dispatchEvent(inputEvent);
            
            // If updateSummary exists (from frontend layout), call it
            if (typeof updateSummary === 'function') {
